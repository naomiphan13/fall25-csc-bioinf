...
class Alignment:
    def __init__ (self, seq1: str, seq2: str):
        self.seq1: str = seq1
        self.seq2: str = seq2
        self._v: int = len(seq1)
        self._w: int = len(seq2)

    def create_score_matrix(self, gap_penalty: int=-2):
        v: int = self._v
        w: int = self._w
    ...
    def global_alignment (self, match_score: int=3, mismatch_score: int=-3, gap_penalty: int=-2):
        matrix: List[List[int]] = self.create_score_matrix()
    ...
    def local_alignment (self, match_score=3, mismatch_score=-3, gap_penalty=-2):
        alignment_matrix: List[List[int]]
        max_score: int
        score: int
    ...
    def fitting_alignment (self, match_score=3, mismatch_score=-3, gap_penalty=-2):
        ...
        alignment_matrix: List[List[int]] = [[0] * (l + 1) for _ in range(s + 1)]
    ...
    def affine_alignment (self, match_score=3, mismatch_score=-3, gap_open_penalty=-5, gap_extension_penalty=-1):
        ...
        lower: List[List[int]] = [[NEG]*(w + 1) for _ in range(v + 1)]
        middle: List[List[int]] = [[NEG]*(w + 1) for _ in range(v + 1)]
        upper: List[List[int]] = [[NEG]*(w + 1) for _ in range(v + 1)]
