import Alignment
from util import read_fasta, data_dir
import os
...
def pairs():
    q = load_map(os.path.join(data_dir(), "q1.fa"))
    t = load_map(os.path.join(data_dir(), "t1.fa"))
    human = load_map(os.path.join(data_dir(), "MT-human.fa"))
    orang = load_map(os.path.join(data_dir(), "MT-orang.fa"))
    for n in range(1, 6):
        qid = f"q{n}"
        tid = f"t{n}"
